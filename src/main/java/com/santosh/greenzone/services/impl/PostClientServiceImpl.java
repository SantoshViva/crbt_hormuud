...
import com.santosh.greenzone.services.PostClientService;
...
public class PostClientServiceImpl implements PostClientService{
	...
	public String sendPostClientReq(String url,String bodyData, String reqType ) {
		...
		try {
			...
			{
				String authHeader= "9FXD299!$G9OBD";
				headers.set("apitoken", authHeader);
			}
			else
			{
			...
			if(response.getStatusCode() == HttpStatus.CREATED) {
				...
				logger.info("response json333="+json.toString());
				...
				if(reqType.toString().equalsIgnoreCase("name"))
				{
					if(json.getString("Message").equalsIgnoreCase("success"))
					{
						...
						logger.info("name="+json.getString("Name"));
						logger.info("name="+nameArr[0]);
						returnValue=nameArr[0];
						logger.info("response result|returnValue="+nameList);
						return nameList;
					}
				}	
